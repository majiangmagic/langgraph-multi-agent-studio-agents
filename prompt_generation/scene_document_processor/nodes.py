...
def constraint_id(polarity: str, value: str) -> str:
    ...
    def __call__(
        ...
        for polarity, values in (
            ...
            for value in values:
                ...
                if not text:
                    continue
                current_constraint_id = constraint_id(polarity, text)
                constraint_entries[current_constraint_id] = {
                    "id": current_constraint_id,
                    "value": text,
                    "polarity": polarity,
                    "status": "active",
        ...
        for removed_constraint_id in proposal.get("removed_constraint_ids") or []:
            if removed_constraint_id in constraint_entries:
                constraint_entries[removed_constraint_id]["status"] = "removed"
                constraint_entries[removed_constraint_id]["removed_by"] = "user"
                constraints_changed = True
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateDocumentNode:
    """Handle the validate document stage."""
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class BuildAgentContextsNode:
    """Handle the build agent contexts stage."""
...
apply_patch_node = ApplyPatchNode()
validate_document_node = ValidateDocumentNode()
build_agent_contexts_node = BuildAgentContextsNode()
