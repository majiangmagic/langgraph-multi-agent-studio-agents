...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class CollectRepairScopeNode:
    """Handle the collect repair scope stage."""
...
class RepairSemanticsNode:
    """Handle the repair semantics stage."""
    ...
    /constraint_overlay/<id> source_path. Do not add new facts."""
            ...
            try:
                ...
                proposed = parse_overlay(str(response.content))
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateRepairNode:
    """Handle the validate repair stage."""
...
collect_repair_scope_node = CollectRepairScopeNode()
repair_semantics_node = RepairSemanticsNode()
validate_repair_node = ValidateRepairNode()
