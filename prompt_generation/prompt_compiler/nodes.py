...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class CollectTermsNode:
    """Handle the collect terms stage."""
    ...
    def __call__(
        ...
        for field in ("identity_terms", "atomic_terms", "relation_terms", "negative_terms"):
            ...
            context[f"prepared_{field}"] = entries(value)
        return {"prepared_context": context}
# </agent-node>
...
def build_term_key(value: Any) -> str:
...
def entries(values: Any) -> list[Dict[str, Any]]:
    result = []
    seen = set()
    for item in values or []:
        ...
        term_key = build_term_key(value)
        if not value or term_key in seen:
            continue
...
def source_paths(item: Dict[str, Any]) -> set[str]:
...
def relation_roots(paths: set[str]) -> set[str]:
    return {
        match.group(1)
        for path in paths
...
def phrase_tokens(value: Any) -> set[str]:
    tokens = re.findall(r"[a-z0-9]+", build_term_key(value))
    stopwords = {
...
def is_verified_term(item: Dict[str, Any]) -> bool:
    return str(item.get("kind") or "").startswith("verified")


def compact_positive_terms(values: Any) -> list[Dict[str, Any]]:
    """Prefer one complete relation phrase while retaining atomic verified tags."""

    term_entries = entries(values)
    relation_indexes: dict[str, int] = {}
    for index, item in enumerate(term_entries):
        ...
        for path in source_paths(item):
            ...
            if previous_index is None or len(phrase_tokens(item["value"])) > len(
                phrase_tokens(term_entries[previous_index]["value"])
            ):
                relation_indexes[relation_path] = index

    canonical_indexes = set(relation_indexes.values())
    canonical_relations = [term_entries[index] for index in canonical_indexes]
    compacted = []
    for index, item in enumerate(term_entries):
        if is_verified_term(item):
        ...
        tokens = phrase_tokens(item.get("value"))
        paths = source_paths(item)
        covered = False
        for relation in canonical_relations:
            if relation is item:
                continue
            relation_tokens = phrase_tokens(relation.get("value"))
            relation_paths = source_paths(relation)
            shares_source = paths_overlap(paths, relation_paths)
            same_relation = bool(
                relation_roots(paths) & relation_roots(relation_paths)
            )
...
def overlay_entry(item: Any, polarity: str) -> Dict[str, Any]:
...
def enrichment_id(item: Dict[str, Any]) -> str:
    identity = "|".join(
        [
            str(item.get("source_path") or ""),
            str(item.get("participant_id") or ""),
            str(item.get("kind") or ""),
            build_term_key(item.get("value")),
        ]
    )
    return "enr_" + hashlib.sha1(identity.encode("utf-8")).hexdigest()[:16]


class CompilePromptNode:
    """Handle the compile prompt stage."""
    ...
    def __call__(
        ...
        identity_terms = list(state.get("prepared_identity_terms") or entries(current_or_previous("identity_terms")))
        atomic_terms = list(state.get("prepared_atomic_terms") or entries(current_or_previous("atomic_terms")))
        relation_terms = list(state.get("prepared_relation_terms") or entries(current_or_previous("relation_terms")))
        negative_terms = list(state.get("prepared_negative_terms") or entries(current_or_previous("negative_terms")))
        positive = entries([*identity_terms, *atomic_terms, *relation_terms])
        negative = entries(negative_terms)
        ...
        for item in positive:
            if not item.get("inferred"):
                continue
            current_enrichment_id = enrichment_id(item)
            previous_entry = previous_entries.get(current_enrichment_id) or {}
            current_entries[current_enrichment_id] = {
                "id": current_enrichment_id,
                "value": item["value"],
            ...
            if not current_entries[current_enrichment_id]["created_document_version"]:
                current_entries[current_enrichment_id]["created_document_version"] = int(
                    (state.get("scene_document") or {}).get("version") or 0
                )
        for previous_id, entry in previous_entries.items():
            if entry.get("status") == "rejected" and previous_id not in current_entries:
                current_entries[previous_id] = entry
        ...
        positive = [
            item
            for item in positive
            if not item.get("inferred") or enrichment_id(item) not in rejected_enrichment_ids
        ]
        ...
        positive = entries(
            [
                ...
                *[
                    ...
                    and not contains_cjk(entry.get("value"))
                ],
            ]
        )
        negative = entries(
        ...
        removed_keys = {build_term_key(value) for value in impact.get("removed_identity_terms") or []}
        positive = [
            item
            for item in positive
            if not any(
                removed and (removed == build_term_key(item["value"]) or removed in build_term_key(item["value"]))
        ...
        if int(overlay.get("document_version") or document_version) != document_version:
            overlay = {}
        remove_positive = {build_term_key(value) for value in overlay.get("remove_positive") or []}
        remove_negative = {build_term_key(value) for value in overlay.get("remove_negative") or []}
        positive = [item for item in positive if build_term_key(item["value"]) not in remove_positive]
        negative = [item for item in negative if build_term_key(item["value"]) not in remove_negative]
        positive = entries(
            [
                *positive,
                *[
                    overlay_entry(item, "positive")
        ...
        positive = compact_positive_terms(positive)
        resolved_ir = {
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidatePromptIrNode:
    """Handle the validate prompt ir stage."""
...
collect_terms_node = CollectTermsNode()
compile_prompt_node = CompilePromptNode()
validate_prompt_ir_node = ValidatePromptIrNode()
