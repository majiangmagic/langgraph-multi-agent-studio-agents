...
def compile_prompt_node(
    ...
    identity_terms = _entries(state.get("identity_terms"))
    atomic_terms = _entries(state.get("atomic_terms"))
    relation_terms = _entries(state.get("relation_terms"))
    negative_terms = _entries(state.get("negative_terms"))
    ...
    seen_records = set()
    for record in [
        *(state.get("identity_tag_records") or []),
        *(state.get("visual_tag_records") or []),
    ]:
    ...
    resolved_ir = {
        "document_version": (state.get("scene_document") or {}).get("version", 0),
        ...
        "identity_tag_records": list(state.get("identity_tag_records") or []),
        "visual_tag_records": list(state.get("visual_tag_records") or []),
