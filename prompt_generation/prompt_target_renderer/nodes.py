...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateRenderInputNode:
    """Handle the validate render input stage."""
...
class RenderPromptNode:
    """Handle the render prompt stage."""
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateRenderResultNode:
    """Handle the validate render result stage."""
...
validate_render_input_node = ValidateRenderInputNode()
render_prompt_node = RenderPromptNode()
validate_render_result_node = ValidateRenderResultNode()
