...
class PrepareContextNode:
    ...
    def __call__(
        ...
        document, previous_ir = load_previous_memory(state)
        return {
            "prepared_context": {
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class PrepareRequestNode:
    """Handle the prepare request stage."""
...
def is_clear_correction(user_input: str) -> bool:
...
def is_unhelpful_initial_clarification(
...
def load_previous_memory(state: SceneDocumentEditorState) -> tuple[Dict[str, Any], Dict[str, Any]]:
...
def active_enrichments(prompt_ir: Dict[str, Any]) -> list[Dict[str, Any]]:
...
def active_constraints(prompt_ir: Dict[str, Any]) -> list[Dict[str, Any]]:
...
async def audit_initial_coverage(
    model: Any,
    user_input: str,
...
meaning is already represented. Return only the JSON object."""
    ...
    audit = SceneCoverageAudit.model_validate(parse_object(str(response.content)))
    return list(dict.fromkeys(value.strip() for value in audit.missing_facts if value.strip()))


def append_required_facts(
...
def visual_fact_count(document: Dict[str, Any]) -> int:
...
class ProposePatchNode:
    """Handle the propose patch stage."""
    ...
    for minors or age-ambiguous participants."""
        model_input = (
            ...
            f"{json.dumps(active_enrichments(previous_ir), ensure_ascii=False)}\n\n"
        ...
        try:
            ...
            for _ in range(2):
                try:
                    ...
                    raw_proposal = parse_object(str(response.content))
                    ...
                    if int(document.get("version") or 0) == 0 and proposal.get("operations"):
                        try:
                            missing_facts = await audit_initial_coverage(
                                model, user_input, candidate
                            )
                        except Exception:
                            missing_facts = (
                                [user_input] if visual_fact_count(candidate) < 2 else []
                            )
                        if missing_facts:
                            proposal = validate_patch_proposal(
                                append_required_facts(proposal, missing_facts),
                                int(document.get("version") or 0),
                            )
                            candidate = apply_patch_proposal(document, proposal)
                    error = ""
                ...
                proposal = fallback_patch(document, user_input, request_id)
        except Exception as exc:
            error = str(exc)
            proposal = fallback_patch(document, user_input, request_id)
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidatePatchNode:
    """Handle the validate patch stage."""
...
prepare_request_node = PrepareRequestNode()
propose_patch_node = ProposePatchNode()
validate_patch_node = ValidatePatchNode()

# Legacy helper aliases for existing integrations.
_append_required_facts = append_required_facts
_fallback_patch = fallback_patch
_is_unhelpful_initial_clarification = is_unhelpful_initial_clarification
_load_previous_memory = load_previous_memory
