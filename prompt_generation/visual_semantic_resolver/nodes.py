...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class PrepareSemanticsNode:
    """Handle the prepare semantics stage."""
...
def parse_visual_extraction(text: str) -> Dict[str, Any]:
...
def parse_visual_decisions(text: str) -> list[Dict[str, Any]]:
...
def authoritative_facts(value: Any) -> list[Dict[str, Any]]:
    return [
        item
...
def path_is_affected(source_path: str, touched_paths: list[str]) -> bool:
...
def merge_incremental_terms(
    ...
    preserved = [
        item
        for item in facts(previous)
        if str(item.get("source_path") or "").strip() != "/summary"
        if not path_is_affected(str(item.get("source_path") or ""), touched_paths)
    ]
    changed = [
        item
        for item in current
        if path_is_affected(str(item.get("source_path") or ""), touched_paths)
    ]
    return [*preserved, *changed]


def fallback_relation_terms(document: Dict[str, Any]) -> list[Dict[str, Any]]:
...
class ResolveVisualSemanticsNode:
    """Handle the resolve visual semantics stage."""
    ...
    async def __call__(
        ...
        if not impact.get("visual_changed") and previous_ir.get("atomic_terms") is not None:
            return {
                "atomic_terms": authoritative_facts(previous_ir.get("atomic_terms")),
                "relation_terms": authoritative_facts(previous_ir.get("relation_terms")),
                "negative_terms": authoritative_facts(previous_ir.get("negative_terms")),
    ...
    suitable for image prompting."""
        ...
        try:
            ...
            parsed = parse_visual_extraction(str(response.content))
        except Exception:
            parsed = {}
        ...
        phrase_values = [
            *[item.get("fallback_phrase") for item in facts(parsed.get("atomic_facts"))],
            *[item.get("phrase") for item in facts(parsed.get("relation_facts"))],
            *[item.get("phrase") for item in facts(parsed.get("negative_facts"))],
        ]
    ...
    Every fallback_phrase and phrase must contain no CJK characters."""
        ...
        for fact in facts(parsed.get("atomic_facts")):
            if contains_cjk(fact.get("fallback_phrase")):
                fact["fallback_phrase"] = ""
        parsed["relation_facts"] = [
            fact
            for fact in facts(parsed.get("relation_facts"))
            if not contains_cjk(fact.get("phrase"))
        ]
        parsed["negative_facts"] = [
            fact
            for fact in facts(parsed.get("negative_facts"))
            if not contains_cjk(fact.get("phrase"))
        ]

        atomic_facts = authoritative_facts(parsed.get("atomic_facts"))
        ...
        resolved_by_input = {
            normalized(item.get("normalized_input")): item
        ...
        for fact_index, fact in enumerate(atomic_facts):
            fact_resolutions = [
                ...
                for candidate in fact.get("candidates") or []
                for item in resolutions
                if normalized(item.get("normalized_input")) == normalized(candidate)
            ]
            ...
            residual_phrase = bool(
                fallback
                and normalized(fallback) not in {normalized(value) for value in matched_names}
            )
    ...
    """
            try:
                ...
                decisions = parse_visual_decisions(str(response.content))
            except Exception as exc:
        ...
        resolved_names = {
            normalized(item.get("name")): normalized(item.get("name"))
            for item in resolutions
            if item.get("name")
        }
        decisions_by_fact = {
        ...
        for fact_index, fact in enumerate(atomic_facts):
            ...
            matched = unique_text(
                [
                    resolved_by_input[normalized(candidate)]["name"]
                    for candidate in fact.get("candidates") or []
                    if normalized(candidate) in resolved_by_input
                ],
                limit=8,
            )
            ...
            if decision:
                ...
                for value in decision.get("selected_tags") or []:
                    ...
                    canonical = resolved_names.get(key) or normalized(
                ...
                if action == "phrase_only":
                    ...
                    matched = unique_text(
                        [
                            resolved_by_input[normalized(value)]["name"]
                            for value in decision.get("retry_candidates") or []
                            if normalized(value) in resolved_by_input
                        ],
                        limit=8,
                    ) or matched
            if matched:
            ...
            if decision and bool(decision.get("preserve_phrase", True)):
                ...
                fallback = ""
            if fallback and normalized(fallback) not in {normalized(value) for value in matched}:
                atomic_terms.append(
                    {
                        "value": fallback,
                        "kind": "descriptive_phrase",
        ...
        relation_terms = [
            ...
            for fact in authoritative_facts(parsed.get("relation_facts"))
            if str(fact.get("phrase") or "").strip()
        ]
        if not relation_terms:
            relation_terms = fallback_relation_terms(document)
        negative_terms = [
            {
                "value": str(fact.get("phrase") or "").strip(),
            ...
            for fact in authoritative_facts(parsed.get("negative_facts"))
            if str(fact.get("phrase") or "").strip()
        ]
        ...
        relation_terms = merge_incremental_terms(
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateVisualResultNode:
    """Handle the validate visual result stage."""
...
prepare_semantics_node = PrepareSemanticsNode()
resolve_visual_semantics_node = ResolveVisualSemanticsNode()
validate_visual_result_node = ValidateVisualResultNode()

# Legacy helper aliases for existing integrations.
_fallback_relation_terms = fallback_relation_terms
_merge_incremental_terms = merge_incremental_terms
_parse_visual_decisions = parse_visual_decisions
