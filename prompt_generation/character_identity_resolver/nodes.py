...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class CollectIdentitiesNode:
    """Handle the collect identities stage."""
...
def parse_identities(text: str) -> list[Dict[str, Any]]:
...
class ResolveIdentitiesNode:
    """Handle the resolve identities stage."""
    ...
    async def __call__(
        ...
        if named:
            ...
            try:
                ...
                resolved = parse_identities(str(response.content))
            except Exception:
                resolved = []
        ...
        initial_by_input = {
            tag(item.get("normalized_input")): item for item in resolutions
        }
        ...
        for item in named:
            ...
            item_resolutions = [
                initial_by_input[tag(value)]
                for value in item_candidates
                if tag(value) in initial_by_input
            ]
            character_matches = [
                resolution
    ...
    """
            try:
                ...
                decisions = parse_decisions(str(response.content))
        ...
        resolved_by_input = {
            tag(record.get("normalized_input")): record
            for record in character_records
        }
        resolved_by_name = {
            tag(record.get("name")): record for record in character_records
        }
        ...
        for item in named:
            ...
            selected = next(
                (
                    tag(resolved_by_input[tag(value)].get("name"))
                    for value in item_candidates
                    if tag(value) in resolved_by_input
                ),
                "",
            )
...
# 2. 只要 DSL 里还保留这个节点名，刷新骨架时会保留本代码块里的业务逻辑。
# 3. 如果新 DSL 删除了这个节点名，生成器会删除整个代码块，即使里面写过业务代码。
class ValidateIdentityResultNode:
    """Handle the validate identity result stage."""
...
collect_identities_node = CollectIdentitiesNode()
resolve_identities_node = ResolveIdentitiesNode()
validate_identity_result_node = ValidateIdentityResultNode()
